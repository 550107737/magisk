...
def check_roman(romanStr, romanRule):
    ...
    return re.search(regex, romanStr)  # 用正则表达式检验输入的罗马数字格式是否正确
...
try:
    if re.match(regex3, instruction):
        ...
        instruction = instruction.lstrip("Please convert").strip()
        ...
        valueStr = instruction.lstrip("Please convert").strip()
